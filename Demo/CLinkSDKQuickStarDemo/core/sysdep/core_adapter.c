...
{
    void *network_handle = ((mbedtls_net_context *) ctx)->handle;
    int32_t ret = g_origin_portfile->core_sysdep_network_send(network_handle, (uint8_t *)buf, len, 5000, NULL);
...
static int32_t _core_mbedtls_net_recv(void *ctx, uint8_t *buf, size_t len)
{
    ...
    int32_t ret = g_origin_portfile->core_sysdep_network_recv(network_handle, buf, len, 5000, NULL);
    if (ret < 0) {
...
{
    void *network_handle = ((mbedtls_net_context *) ctx)->handle;
    int32_t ret = g_origin_portfile->core_sysdep_network_recv(network_handle, buf, len, timeout, NULL);
...
int32_t _tls_network_establish(void *handle)
{
    ...
    adapter_handle->mbedtls.net_ctx.handle = adapter_handle->network_handle;
    adapter_handle->mbedtls.net_ctx.fd = 1;
    ...
    mbedtls_ssl_set_bio(&adapter_handle->mbedtls.ssl_ctx, &adapter_handle->mbedtls.net_ctx, _core_mbedtls_net_send,
